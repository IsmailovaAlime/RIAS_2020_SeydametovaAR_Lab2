...
public class Client {
      ...
      if (command.contains("send") || command.contains("s/")) {
        if (command.contains(" ")) {
          if (!client.isClosed()) {
            ...
            System.out.println("С ума сошел что ли, ты же не подключен к серверу)");
          logger.warn("Don't connected to server");
        } else
          wrongСommand();
      }else if (command.contains("disconnect")||command.contains("d/")) {
          System.out.println("Connection terminated: 127.0.0.1 / 5555");
  ...
  public static void wrongСommand() {
    System.out.println("I don't know this command. Enter \"help\" for reference");
  ...
  public static void quit(String command,  PrintStream output,  Socket client) throws IOException {
      ...
      System.out.println("Application exit! Bye :D");
